    ...
    Column(modifier = modifier.fillMaxSize()) {
        Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 8.dp)) {
            ...
            Button(
                ...
                val streakSuffix = if (uiState.dailyChallengeState.currentStreak > 0) {
                    " (🔥${uiState.dailyChallengeState.currentStreak})"
                } else {
                    ""
                }
                Text(if (uiState.isDailyChallengeCompletedToday) "✓ Reto del día" else "☀ Reto del día$streakSuffix")
            }
        }
